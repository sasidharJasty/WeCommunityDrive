...
export default function Event() {
  ...
  async function fetchData() {
    try {
      ...
      const YourEvents = await axios.get(
        `http://127.0.0.1:8000/04D2430AAFE10AA4/events/?user_id=${usrData.User}`,
        {
          headers: {
  ...
  useEffect(() => {
    if (usrData["userType"] !== "Organizer") { /* Stop people who are not organizers from making events */
      history("/");
    }
    ...
    <>
      ...
      <div className="absolute left-10 w-2/5 top-20 mt-20 scrollable-container">
        {events.map((obj,key) => (
          ...
          <div class="notification mb-3 " onClick={(e) => setIndex(key)} key={key}>
          <div class="notiglow"></div>
          <div class="notiborderglow"></div>
          <div class="notititle">{obj["Event_Name"]}</div>
          <div class="notibody">{obj["Event_Description"]}</div>
          <div class="notibody">{obj["Event_Location"]}, by {obj["Username"]}</div>
        </div>
        ))}
      </div>
  ...
  <div className="notifications-container rounded-3xl w-2/5 ml-72 border absolute left-96 top-36 border-4 border-green-200">
    <div className="success">
      ...
      <div className={`main2 left-[50vw] absolute w-11/12 mt-10 ${popup ? '' : 'hidden'}`}>
        ...
        <div className="signup">
          <form onSubmit={handleEvent} className="mb-20 form">
            ...
            <div className="w-[40vw]">
            <label className="placeholder-black !text-[1.2vw] mt-0 font-semibold text-black">Event Start Time</label>
            <input
              className="py-5 m-auto mb-5 placeholder-black"
              type="datetime-local"
              name="pswd"
            ...
            />
            <label className="placeholder-black !text-[1.2vw] mt-0 font-semibold text-black">Event End Time</label>
            <input
              className="py-5 m-auto mb-5 placeholder-black"
              type="datetime-local"
              name="pswd"
            ...
            <button className="m-auto mt-[5vh] button ml-5 bg-red-400 !border-red-400 text-white hover:!text-red-400" onClick={()=>setPopup(false)}>
              Close
            </button>
            </div>
            </div>
